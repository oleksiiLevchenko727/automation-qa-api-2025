...
afterEach(async () => {
 ...
 test("Should get list of cars and validate structure", async () => {
    ...
    createdCarId = createResponse.data.data.id; 

    const response = await carsController.getCars();
    expect(response.status).toBe(200);
    expect(Array.isArray(response.data.data)).toBe(true);

    const car = response.data.data.find(c => c.id === createdCarId);
    expect(car).toEqual({
